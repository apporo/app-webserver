...
var Service = function(params) {
  ...
  var positionInstance = new (function() {
    ...
    this.POSITION_TRACE_REQUEST = -116;
    this.POSITION_TRACE_SESSION = -115;
  ...
  routers.push({
    ...
    priority: positionInstance.POSITION_TRACE_REQUEST
  }, {
    name: 'trace-session',
    middleware: requestTracer({ scope: 'session' }),
    priority: positionInstance.POSITION_TRACE_SESSION
  }, {
    name: 'trace-logging',
    middleware: function(req, res, next) {
      self.logger.debug('Req[%s] belongs to Session[%s] - SessionID[%s]',
          req.traceRequestId, req.traceSessionId, req.sessionID);
      req.on('end', function() {
        self.logger.debug('Req[%s] has finished', req.traceRequestId);
      });
      next();
    },
