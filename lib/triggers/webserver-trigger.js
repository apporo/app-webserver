...
var Service = function(params) {
  ...
  debuglog(' - session store: %s', JSON.stringify(sessionStoreDef));
  ...
  switch(sessionStoreDef.type) {
    case 'redis':
      ...
      debuglog(' - session.store ~ redisStore');
      break;
    case 'mongodb':
      sessionOpts.store = new mongoStore({
        url: sessionStoreDef.url
      });
      debuglog(' - session.store ~ mongoStore');
      break;
    default:
      debuglog(' - session.store ~ MemoryStore (default)');
  }
  ...
  var positionInstance = new (function() {
    ...
    this.POSITION_REQUEST_TRACER = -115;
    this.POSITION_LOGGING_TRACE = -114;
  ...
  var tracerInstance = express.Router();

  var tracerOf = {}
  lodash.forEach(['process', 'session', 'request'], function(myScope) {
    tracerOf[myScope] = requestTracer({ scope: myScope });
    tracerInstance.use(tracerOf[myScope].interceptor());
  });

  tracerOf['process'].on('traceIdCreated', function(traceProcessId) {
    self.logger.debug('New process[%s] with context: %s', traceProcessId, JSON.stringify(webserverConfig));
  });

  tracerOf['session'].on('traceIdCreated', function(traceSessionId, traceProcessId) {
    self.logger.debug('New session[%s] in process[%s]', traceSessionId, traceProcessId);
  });

  tracerOf['request'].on('traceIdCreated', function(traceRequestId, traceSessionId) {
  ...
  routers.push({
    ...
    name: 'request-tracer',
    middleware: tracerInstance,
    priority: positionInstance.POSITION_REQUEST_TRACER
  }, {
    name: 'trace-logging',
    ...
    priority: positionInstance.POSITION_LOGGING_TRACE
  }, {
