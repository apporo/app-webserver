...
var Service = function(params) {
  ...
  var legacyMode = webserverConfig.legacyMode;
  Object.defineProperty(self, 'legacyMode', {
    get: function() { return legacyMode },
    set: function(value) { legacyMode = value }
  });

  var appHost = webserverConfig && webserverConfig.host || '0.0.0.0';
  var appPort = webserverConfig && webserverConfig.port || 7979;
  var appProto = 'http';

  var sslEnabled = false;
  if (webserverConfig.ssl && webserverConfig.ssl.enabled) {
    webserverConfig.ssl = webserverConfig.ssl || {};

    var caText = webserverConfig.ssl.ca;
    try {
      caText = caText || fs.readFileSync(webserverConfig.ssl.ca_file);
    } catch(error) {
      debugx.enabled && debugx('error on loading CA files: %s', JSON.stringify(error));
    }

    var keyText = webserverConfig.ssl.key;
    var certText = webserverConfig.ssl.cert;
    try {
      keyText = keyText || fs.readFileSync(webserverConfig.ssl.key_file);
      certText = certText || fs.readFileSync(webserverConfig.ssl.cert_file);
    } catch(error) {
      debugx.enabled && debugx('error on loading key/cert files: %s', JSON.stringify(error));
    }

    if (!keyText && !certText && SERVER_HOSTS.indexOf(appHost)>=0) {
      keyText = fs.readFileSync(path.join(__dirname, '../../data/ssl/localhost.key.pem'));
      certText = fs.readFileSync(path.join(__dirname, '../../data/ssl/localhost.cert.pem'));
    }
  ...
  self.start = function() {
    ...
    return new Promise(function(resolved, rejected) {
      ...
      var serverInstance = server.listen(appPort, appHost, function () {
        ...
        (webserverConfig && webserverConfig.verbose !== false || debuglog.enabled) &&
