...
var debugx = debug('appWebserver:trigger');
var fs = require('fs');
...
var Service = function(params) {
  ...
  if (webserverConfig.ssl && webserverConfig.ssl.enabled) {
    ...
    ssl.ca = webserverConfig.ssl.ca;
    try {
      ssl.ca = ssl.ca || fs.readFileSync(webserverConfig.ssl.ca_file);
    } catch(error) {
      debugx.enabled && debugx('error on loading CA files: %s', JSON.stringify(error));
    }

    ssl.key = webserverConfig.ssl.key;
    ssl.cert = webserverConfig.ssl.cert;
    try {
      ssl.key = ssl.key || fs.readFileSync(webserverConfig.ssl.key_file);
      ssl.cert = ssl.cert || fs.readFileSync(webserverConfig.ssl.cert_file);
    } catch(error) {
  ...
  if (debugx.enabled && webserverConfig.printRequestInfo) {
  ...
  self.start = function() {
    ...
    return new Promise(function(resolved, rejected) {
      ...
      var serverInstance = server.listen(appPort, appHost, function () {
        ...
        (webserverConfig && webserverConfig.verbose !== false || debugx.enabled) &&
