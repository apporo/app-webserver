...
var Service = function(params) {
  ...
  var tracerListener = requestTracer({ express: express });

  tracerListener.on('newProcess', function(traceProcessId) {
    self.logger.debug('New process[%s] with configuration: %s', traceProcessId, JSON.stringify(webserverConfig));
  });

  tracerListener.on('newSession', function(traceSessionId, traceProcessId, ext) {
    self.logger.debug('New session[%s] in process[%s], sessionID: %s', traceSessionId, traceProcessId, ext.sessionID);
  });

  tracerListener.on('newRequest', function(traceRequestId, traceSessionId) {
  ...
  routers.push({
    ...
    name: 'request-tracer',
    middleware: tracerListener.interceptor(),
